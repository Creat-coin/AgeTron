...
void QRCodeDialog::setModel(OptionsModel *model)
{
    ...
    // update the display unit, to not use the default ("AGT")
    updateDisplayUnit();
}
...
{
    QString ret = QString("Agetron:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non AGT unit input in lnReqAmount, we generate the URI with AGT as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::AGT, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
